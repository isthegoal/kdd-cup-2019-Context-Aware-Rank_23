        ...
        for stations in stationsNeeded:
            ...
            for dates in datesNeeded:
                ...
                if (stations, dates) not in alreadyHave:
                    ...
                    cached = pd.concat([cached, get_met_data(dates, 1, api_key, float(31.213), float(121.445), str(stations) )])

                    print('the  cached is:' ,get_met_data(dates, 1, api_key, float(31.213), float(121.445), str(stations)))
...
getWeatherDataRange('', '', ['xujiahui_meo'], 'Shanghai', shortRun = True)
