...
data_path = './'
print('基本数据集加载')
data = pd.read_csv(data_path + 'data_shanghai.csv',parse_dates=['req_time','plan_time'])
...
##########################################  协同特征  #############################################
xietong_data = pd.read_csv('./xietong_feature_shanghai.csv', index_col = 0)
...
#保存所有特征
filename="shanghai_{:%Y-%m-%d_%H_%M}_all_features.csv".format(datetime.now())
...
filename="shanghai_{:%Y-%m-%d_%H_%M}_Select_features.csv".format(datetime.now())
