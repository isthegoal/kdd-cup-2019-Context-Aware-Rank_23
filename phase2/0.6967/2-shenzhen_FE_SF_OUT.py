...
##########################################  协同特征  #############################################
xietong_data = pd.read_csv('./xietong_feature_shenzhen.csv', index_col = 0)
...
#保存所有特征
filename="shenzhen_{:%Y-%m-%d_%H_%M}_all_features.csv".format(datetime.now())
...
filename="shenzhen_{:%Y-%m-%d_%H_%M}_Select_features.csv".format(datetime.now())
